...
COMMODITY_PATH = r'C:\Users\Julio\Documents\Servers\Server Maplestory\Cosmic-master\Cosmic-master\wz\Etc.wz\Commodity.img.xml'

def parse_commodity():
    tree = ET.parse(COMMODITY_PATH)
    root = tree.getroot()
    items = []
    for child in root:
        entry = {}
...
def get_item_names_map():
    ...
    result = {r['itemid']: {'name': r['name'], 'category': r['category']} for r in cursor.fetchall()}
    cursor.close()
    db.close()
    return result

@app.route('/api/cashshop/items')
def api_cashshop_items():
    try:
...
@app.route('/api/cashshop/update', methods=['POST'])
def api_cashshop_update():
    try:
        data = request.json  # {sn, price, period, onSale, count}
        sn = int(data['sn'])
        ...
        tree.write(COMMODITY_PATH, encoding='unicode', xml_declaration=False)
        return jsonify({'ok': True})
    except Exception as e:
        return jsonify({'error': str(e)}), 500

@app.route('/api/cashshop/add', methods=['POST'])
def api_cashshop_add():
    try:
        data = request.json  # {itemId, price, period, count, onSale}
        tree, root, items = parse_commodity()
        # Gera novo SN único (max + 1)
...
@app.route('/api/cashshop/remove', methods=['POST'])
def api_cashshop_remove():
    try:
...
@app.route('/api/cashshop/search_item')
def api_cashshop_search_item():
